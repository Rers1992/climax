class Dashboard extends React.Component {
    ...
    componentDidMount() {
        ...
        fetch('../memoria/regionJson')
            .then(response => response.json())
            .then(data => this.setState({regiones: data.regiones, estaciones: data.estaciones}))
    }
        ...
        return <div className="row">
            {data.map((dato) => (
                <div className="col-12 col-12 col-md-12 col-lg-12 col-xl-12">
                    <div className="card card-photo contenedor" onClick={() => this.cambioUbicacionMapa(dato.latitud, dato.longitud, dato.codigoregion)}>
                        <div className="card-body">
                            <h5 className="card-title">{dato.nombreregion}</h5>
                        </div>
                    </div>
                </div>
            ))}
    ...
    render() {
        if (this.state.codigoEstacion != 0) {
            fetch('../memoria/estacionesRegion/' + this.state.codigoEstacion)
                .then(response => response.json())
                .then(data => this.setState({
                    data: data.indices, longitud: data.long, latitud: data.lat,
            ...
            <div className="col-3 col-12 col-md-12 col-lg-6 col-xl-3 pb-4">
                <div className="card card-style overflow-auto altoMax">
                    <div className="card-body">
                        ...
                        <div className="row">
                            {this.cargarListaEstaciones(this.state.regiones)}
                        </div>
                    </div>
                </div>
            </div>
